...
除本文件的 Token 与组件规范外，前台页面的视觉风格统一参考：

- `E:\ZZY_PROJECT\crmeb_java\docs\stitch_`

该目录下已经包含首页、商城、分类、详情、案例、下载、关于我们、联系我们等参考稿。  
后续前台页面开发必须遵守以下原则：

- 先遵守本文件的 Token、组件、布局规范
- 再以 `stitch_` 中对应页面的版式、信息密度、配色气质、卡片结构作为直接参考
- 不允许开发人员脱离 `stitch_` 参考稿自行发挥另一套页面风格
